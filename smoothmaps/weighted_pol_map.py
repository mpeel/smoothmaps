...
def weighted_pol_map(nside=512,indirectory='',outdirectory='',date='',prefix='',index=-3.0,freqs=[],maps=[],maps_half1=[],maps_half2=[],use_halfrings=False,use_weights=False,use_reweight_by_rms=False,use_reweight_by_rms_method=2,use_planck=True,use_cbass=False,normfreq=10.0,rescale_amp=[],rescale_variance=[],apply_extra_mask=[],extra_mask='',threshold=1.0,varianceindex=[],separate_variance_maps=[]):
	...
	for i in range(0,nummaps):
		...
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_0.pdf')
		plt.close()
		plt.clf()
		hp.mollview(mapdata[1],norm='hist')
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_1.pdf')
		plt.close()
		plt.clf()
		hp.mollview(mapdata[2],norm='hist')
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_2.pdf')
		plt.close()
		plt.clf()
		...
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_1_rescale.pdf')
		plt.close()
		plt.clf()
		hp.mollview(temp_U,min=-threshold,max=threshold)
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_2_rescale.pdf')
		plt.close()
		plt.clf()
		...
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_1_var.pdf')
		plt.close()
		plt.clf()
		hp.mollview(var_u,norm='hist')
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_2_var.pdf')
		plt.close()
		plt.clf()
		...
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_1_var_rescale.pdf')
		plt.close()
		plt.clf()
		hp.mollview(var_u*rescale_variance[i],min=umin*(normfreq/freqs[i])**index,max=umax*(normfreq/freqs[i])**index)#,norm='hist')
		plt.savefig(outdirectory+maps[i].split('/')[-1]+'_2_var_rescale.pdf')
		plt.close()
		plt.clf()
